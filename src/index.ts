...
import { getHistory, saveHistory, deleteSession, sessionCount } from "./sessionStore.js";
...
/**
 * Have the model phrase the missing ladder rung as a natural conversational
 * turn — acknowledging the user's last answer instead of dropping a canned
 * sentence on them. One extra model call, only on the (rare) guard-trigger
...
async function naturalLadderQuestion(
  ...
  try {
    ...
    if (!text) return null;
    if (/\[\[expert:/.test(text)) return null;       // recommended anyway
    if (/\*\*[^*\n]+\*\*\s*—/.test(text)) return null; // card-shaped header
    if (!text.includes("?")) return null;             // not a question
    if (text.length > 600) return null;               // rambling
    return text;
  } catch (err) {
    console.error("[guard] naturalLadderQuestion error:", err);
    return null;
  }
}
...
async function guardResponse(
  ...
  // rung before showing them. Seen in prod twice: "…online ya da İstanbul'da
  // uygun 3 terapist" (modality AND city invented), and cards shown right
  // after "yüzyüze" with the city never asked.
  //
  // The replacement question is generated by the model itself (a real
  // conversational turn that acknowledges what the user just said) so the
  // dialogue keeps its chat rhythm; the static LADDER_* texts are only the
  // fallback when that generation fails or misbehaves.
  if (userMessage) {
    try {
      ...
      if (!ladder.skipped && ladder.reason && ladder.reason !== "no_cards") {
        console.log(
          `[guard] ladder pass: ${ladder.reason} (history=${history.length} turns)`
        );
      }
      if (ladder.skipped && ladder.missingRung) {
        violations.push({
          kind: "other",
          detail: `ladder_step_skipped:${ladder.missingRung}`,
        });
        console.warn(
          `[guard] ladder step skipped — asking ${ladder.missingRung} instead`
        );
        try {
          Sentry.captureMessage("Ladder step skipped — asked question", {
            level: "warning",
            tags: {
              kind: "ladder_skip",
              rung: ladder.missingRung,
              intent: intent?.intent ?? "unknown",
              tool_count: String(toolCallCount),
            },
          });
...
async function runHttp(): Promise<void> {
  ...
  app.post("/v1/assistant/session/reset", requireApiKey, async (req: Request, res: Response) => {
    const sessionId =
      typeof req.body?.session_id === "string" && UUID_RE.test(req.body.session_id.trim())
