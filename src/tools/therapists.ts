...
function normaliseListResponse(
  ...
  const total: number =
    raw.total ?? raw.count ?? therapists.length;
  const totalPages: number =
    raw.total_pages ?? Math.ceil(total / per_page);
...
function therapistToMarkdown(t: Therapist, index?: number): string {
  ...
  if (t.title) lines.push(`**Unvan:** ${t.title}`);
  ...
  if (t.languages?.length) lines.push(`**Diller:** ${t.languages.join(", ")}`);

  const location = [t.city, t.country].filter(Boolean).join(", ") || t.location;
  if (location) lines.push(`**Konum:** ${location}`);

  if (t.online !== undefined) lines.push(`**Online:** ${t.online ? "Evet" : "Hayır"}`);
  ...
  if (t.rating !== undefined) {
    lines.push(
      `**Puan:** ${t.rating}${t.review_count !== undefined ? ` (${t.review_count} değerlendirme)` : ""}`
    );
  }
  ...
  if (t.gender) lines.push(`**Cinsiyet:** ${t.gender}`);

  const approaches: string[] = Array.isArray(t.approach)
    ? t.approach
    : t.approach
    ? [t.approach]
    : [];
  if (approaches.length) lines.push(`**Yaklaşım:** ${approaches.join(", ")}`);

  if (t.bio) lines.push(`\n> ${t.bio.replace(/\n/g, "\n> ")}`);

  if (t.profile_url) lines.push(`\n🔗 [Profil](${t.profile_url})`);
