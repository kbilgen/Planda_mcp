...
export const ListInputSchema = PaginationSchema.merge(FilterSchema)
  .merge(FormatSchema)
  .strict();
...
export const AvailableDaysInputSchema = z
  .object({
    therapist_id: z
      .union([z.string(), z.number()])
...
// ─── Tool descriptions (shared by MCP registration and local agent tools) ────

export const FIND_THERAPISTS_DESCRIPTION = `Search and list licensed therapists from Planda (planda.org), Turkey's leading therapy marketplace.
...
export const GET_THERAPIST_DESCRIPTION = `Fetches the full profile of a single therapist.
...
export const LIST_SPECIALTIES_DESCRIPTION = `Returns all therapy specialty areas available on the Planda marketplace.
...
export const GET_HOURS_DESCRIPTION = `Returns bookable appointment time slots for a therapist on a specific date.
...
export const GET_DAYS_DESCRIPTION = `Returns the dates (days) on which a therapist has availability at a specific branch.
...
export const GET_ACTIVE_CITIES_DESCRIPTION = `Returns the list of cities where Planda therapists are currently active.
...
export async function handleFindTherapists(
  params: ListInput
): Promise<ToolHandlerResult<TherapistListOutput>> {
  try {
    ...
    let inDistrictCount: number | null = null;
    let nearbySuggestions: NearbyDistrictSuggestion[] = [];
    ...
    if (hasPostFilter) {
      ...
      if (districtFilter) {
        ...
        inDistrictCount = beforeDistrict.filter((t) =>
        ...
        if (inDistrictCount === 0) {
          ...
          nearbySuggestions = suggestNearbyDistricts(
            beforeDistrict,
            districtFilter
          );
        }
    ...
    const districtNote =
      districtFilter && inDistrictCount === 0
    ...
    if (!output.therapists.length) {
      return {
        text:
          "Belirtilen kriterlere uygun terapist bulunamadı." +
          (districtNote ? `\n\n${districtNote}` : ""),
        structured: output,
      };
    }
    ...
    return { text: handleApiError(error), structured: undefined as never, isError: true };
  }
}

export async function handleGetTherapist(
  params: GetInput
): Promise<ToolHandlerResult<Therapist>> {
  try {
    ...
    const candidate = raw as Partial<Therapist> & { data?: unknown };
    ...
    return { text, structured: therapist };
  } catch (error) {
    return { text: handleApiError(error), structured: undefined as never, isError: true };
  }
}

export async function handleListSpecialties(): Promise<
  ToolHandlerResult<{ specialties: unknown[] }>
> {
  try {
...
export async function handleGetTherapistHours(
  params: HoursInput
): Promise<ToolHandlerResult<{ date: string; slots: unknown[] }>> {
  try {
    ...
    if (!slots.length) {
      return {
        text: `${params.date} tarihinde müsait saat bulunamadı.`,
        structured: { date: params.date, slots: [] },
      };
    }
...
export async function handleGetTherapistAvailableDays(
  params: AvailableDaysInput
): Promise<ToolHandlerResult<{ days: unknown[] }>> {
  try {
    ...
    if (!days.length) {
      return {
        text: "Bu şube için müsait gün bulunamadı.",
        structured: { days: [] },
      };
    }
...
export async function handleGetActiveCities(): Promise<
  ToolHandlerResult<{ cities: string[] }>
> {
  try {
    ...
    if (!cities.length) {
      return {
        text: "Aktif şehir listesi alınamadı.",
        structured: { cities: [] },
      };
    }
    ...
    return {
      text: `# Planda Aktif Şehirler\n\n${names.map((n) => `- ${n}`).join("\n")}`,
      structured: { cities: names },
    };
  } catch (error) {
    return { text: handleApiError(error), structured: undefined as never, isError: true };
  }
