...
export function districtDisplayName(district: string): string {
  const key = normTR(district);
  return DISTRICT_DISPLAY[key] ?? district;
}

export interface NearbyDistrictSuggestion {
  district: string;        // normalized key, e.g. "bahcelievler"
  display_name: string;    // "Bahçelievler"
  therapist_count: number; // therapists with a physical branch there
}
 ...
 */
export function suggestNearbyDistricts(
  therapists: Therapist[],
  district: string
): NearbyDistrictSuggestion[] {
  const key = normTR(district);
  const neighbours = NEARBY_DISTRICTS[key] ?? [];
  const out: NearbyDistrictSuggestion[] = [];
...
export function mentionsLocation(text: string): boolean {
  ...
  for (const w of words) {
    if (TR_PROVINCES.has(w)) return true;
  }
