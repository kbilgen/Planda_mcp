...
// ─── In-memory approaches cache ──────────────────────────────────────────────
// Per-therapist; 5-minute TTL. Avoids re-fetching when the same approach
// search runs back-to-back (e.g. two users asking BDT in the same window).
...
const cache = new Map<string | number, ApproachEntry>();

function cacheGet(id: string | number): Approach[] | null {
...
function cacheSet(id: string | number, approaches: Approach[]): void {
...
const APPROACH_RULES: Array<{ userKeys: string[]; therapistSubstr: string[] }> = [
...
function approachSubstrings(query: string): string[] {
  const q = normTR(query);
  if (!q) return [];
  for (const rule of APPROACH_RULES) {
    if (rule.userKeys.some((k) => q.includes(k))) return rule.therapistSubstr;
  }
...
function approachMatches(approaches: Approach[], query: string): boolean {
 ...
 */
async function fetchApproaches(id: string | number): Promise<Approach[]> {
 ...
 */
export async function filterByApproachVerified(
  ...
  const results: { t: Therapist; matches: boolean }[] = new Array(candidates.length);
  ...
  for (let w = 0; w < Math.min(concurrency, candidates.length); w++) {
    workers.push((async () => {
      while (true) {
        ...
        // fetch.
        if (approaches.length > 0) t.approaches = approaches;
