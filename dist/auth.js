...
const VALIDATE_TEMPLATE = process.env.PLANDA_AUTH_ENDPOINT_TEMPLATE ??
...
function cacheKey(token, userId) {
 ...
 */
export async function validatePlandaToken(token, userId) {
    ...
    if (redis) {
        try {
            ...
            console.warn("[auth] Redis cache read error:", err.message);
        }
    }
    ...
    try {
        ...
        console.warn("[auth] Planda token validation failed:", err.message);
        result = { valid: false };
    }
    if (redis && result.valid) {
        try {
            ...
            console.warn("[auth] Redis cache write error:", err.message);
        }
    }
    return result;
}
