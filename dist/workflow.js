...
⚠️ MANDATORY for approach queries (BDT, EMDR, ACT, Schema, Gestalt etc.):
  ...
  - approaches[] empty/null or call fails → EXCLUDE, never guess`,
        input_schema: {
            type: "object",
            properties: {
                id: { type: ["string", "number"], description: "Therapist numeric ID (use username instead if available)" },
                username: { type: "string", description: "Therapist username slug (e.g. gulcin_yilmaz) — preferred over id" },
            },
        },
    },
    {
