...
async function requirePlandaAuth(req, res, next) {
...
async function runHttp() {
    ...
    //
    app.post("/v1/assistant/chat", requirePlandaAuth, async (req, res) => {
    ...
    //
    app.post("/v1/assistant/chat/stream", requirePlandaAuth, async (req, res) => {
        if (!process.env.OPENAI_API_KEY) {
            res.status(500).json({ error: "OPENAI_API_KEY not configured" });
            return;
        }
