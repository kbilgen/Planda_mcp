...
async function naturalLadderQuestion(rung, userMessage, history) {
    ...
    try {
        ...
        if (!text)
            return null;
        if (/\[\[expert:/.test(text))
            return null; // recommended anyway
        if (/\*\*[^*\n]+\*\*\s*—/.test(text))
            return null; // card-shaped header
        if (!text.includes("?"))
            return null; // not a question
        if (text.length > 600)
            return null; // rambling
        return text;
    }
    catch (err) {
    ...
    if (userMessage) {
        try {
            ...
            if (ladder.skipped && ladder.missingRung) {
                ...
                console.warn(`[guard] ladder step skipped — asking ${ladder.missingRung} instead`);
                try {
...
async function runHttp() {
    ...
    app.post("/v1/assistant/chat/stream", requireApiKey, async (req, res) => {
        ...
        try {
            let fullText = "";
            let { updatedHistory, toolCalls, model } = await runChatStream({ message, history, forceToolCall }, {
